...
function updateTextBasedOnSize() {
  ...
  const isMobile =
    368 >= window.innerWidth <= 468 && 480 >= window.innerHeight <= 642;
  ...
  spans.forEach((span) => {
    if (isMobile) {
      if (
        span.getAttribute("data-text") === "Elève ingénieur en informatique"
      ) {
        ...
        span.getAttribute("data-text") === "Développeur Mobile junior"
      ) {
    ...
    contactForm.addEventListener("submit", function (event) {
      ...
      fetch("https://formspree.io/f/xvgzdjad", {
        ...
        .catch((error) => {
          ...
          formMessage.textContent =
            "⚠️ Erreur réseau. Vérifiez votre connexion.";
          formMessage.style.color = "red";
        });
    });
  } else {
